...
```{r}
test_20_21_raw = read.csv("http://www.football-data.co.uk/mmz4281/9394/E0.csv")
...
while (first_year < 2020) {
  ...
  data = read_csv(url)%>%
    ...
    mutate(goal_difference = case_when(venue == "HomeTeam" ~ (FTHG - FTAG),
                                       venue == "AwayTeam" ~ (FTAG - FTHG)),
           Team = paste(Team, first_year_trimmed, second_year_trimmed,
                        sep = "_")) %>%
    group_by(Team) %>%
    mutate(Gameweek = row_number(),
           goal_difference_running = cumsum(goal_difference)) %>%
    select(Gameweek, Team, goal_difference_running) %>%
    pivot_wider(id_cols = Gameweek, names_from = Team, values_from = goal_difference_running)
  
  #assign(paste("GD_Evolution", first_year_trimmed, second_year_trimmed, sep="_"), data)
  season_name = paste(first_year, second_year_trimmed, sep = "-")
  all_dataframes = c(all_dataframes, season_name = list(data))
  names(all_dataframes)[length(all_dataframes)] = season_name
...
## COUNT AND ORDER ALL CHASES BETWEEN ALL TEAMS IN PL HISTORY
...
#To do chases b date, rather than gameweek, every club needs a total for...
#every game date throughout the season, based on games they have played to that point
#idea: create full time series of dates for every club, the fill everything downwards
...
all_gd_pts_deficits = all_gd_pts_evolution %>%
  ...
  #or Date
    ...
    rename(first_team = Team.x, second_team = Team.y, 
           ...
           first_team_deficit_pts_1 = first_team_deficit_pts.x, first_team_deficit_pts_2 = first_team_deficit_pts.y,
...
all_gd_chases = all_gd_pts_deficits %>%
    #filter so you look at genuine chases
    #I.e. initial gap (i.e. positive) and second gap is larger (i.e. polarity is reversed)
    #Atm this just looks at gd chases
    filter(first_team_deficit_2 < first_team_deficit_1,
           first_team_deficit_2 <= 0,
           first_team_deficit_1 >0)

all_pts_chases = all_gd_pts_deficits%>%
    #filter so you look at genuine chases
    #I.e. initial gap (i.e. positive) and second gap is larger (i.e. polarity is reversed)
    #Atm this just looks at gd chases
    filter(first_team_deficit_pts_2 < first_team_deficit_pts_1,
           first_team_deficit_pts_2 <= 0,
           first_team_deficit_pts_1 >0)
...
#biggest_gd_chases_by_season = biggest_gd_chases_by_season %>%
#    bind_rows(biggest_gd_chases)
