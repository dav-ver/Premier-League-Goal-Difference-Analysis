...
# Create blank summary dataframes to fill with the loop
...
all_gd_pts_evolution = read_csv(url) %>%
    ...
    mutate(gd = case_when(venue == "HomeTeam" ~ (FTHG - FTAG),
           ...
           Team = paste(Team, first_year_trimmed, second_year_trimmed,
                        sep = "_"),
           gls = case_when(venue == "HomeTeam" ~ FTHG,
                           venue == "AwayTeam" ~ FTAG)) %>%
    group_by(Team) %>%
    mutate(Gameweek = row_number(),
           gd_running = cumsum(gd),
           pts_running = cumsum(pts),
           gls_running = cumsum(gls)) %>%
    select(date, Gameweek, Team, gd_running, pts_running, gls_running) %>%
  ...
  group_by(date) %>%
  arrange(desc(pts_running), desc(gd_running), desc(gls_running)) %>%
